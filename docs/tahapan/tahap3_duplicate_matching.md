...
> Catatan penting: di referensi (`Data Profiling (1).ipynb`), matching dicari **dalam satu dataset** (cari WP duplikat). Untuk project ini, matching utama dilakukan **antar dua dataset** (OSS vs CEISA), sesuai prioritas di [`../02_business_rules.md`](../02_business_rules.md) §1.
...
4. **Exact match — Prioritas 2 (NPWP)**: untuk record yang belum match di langkah 3, join pada `npwp_digits` → `match_type=EXACT_NPWP`
5. **Fuzzy matching — Prioritas 3**: untuk sisa record yang belum match, hitung similarity `nama_key` (Jaro-Winkler, token sort/set ratio) dan kemiripan alamat
...
9. **Duplicate clustering** (internal per sumber): gunakan `networkx` untuk grouping record yang merujuk entitas sama dalam satu sumber (mis. NIB sama muncul >1 kali di OSS)
